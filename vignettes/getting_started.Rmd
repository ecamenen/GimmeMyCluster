...
```{r}
sil <- getSilhouettePerPart(dist, cls) %>%
...
```{r}
between <- calculate_inertia(df_scaled, cls)
plotElbow(between)
```

Quality checks could be summarized in a table.
```{r}
calculate_between_diff(between) %>%
  printSummary(between, ., sil) %>% 
  pretty_table()
```

## Visualizing the Clusters
Principal Component Analysis (PCA) reduces the dataset to two dimensions for easy visualization of cluster separation. For example, sample 42, which was flagged as misclassified by the Silhouette index, appears closer to the other irises group in the PCA plot.
```{r}
...
```{r}
heatMap(df_scaled, dist = dist, c = clustering, cl = cl)
```
...
```{r}
getPdisPerPartition(df_scaled, cls) %>% 
  multiply_by(100) %>% 
  pretty_table()
```
...
```{r}
getDiscriminantVariables(df_scaled, cl) %>%
...
```{r}
calculate_centroids(df, cl) %>% round(1)
```
